...
var Client = /** @class */ (function () {
    function Client(url, options, connectOptions) {
        if (options === void 0) { options = {}; }
        if (connectOptions === void 0) { connectOptions = {}; }
        var _this = this;
        // signals
        ...
        this.hostname = url;
        Storage_1.getItem('colyseusid', function (colyseusid) { return _this.connect(colyseusid, options, connectOptions); });
    }
    Client.prototype.join = function (roomName, options, reuseRoomInstance, retryCount) {
        if (options === void 0) { options = {}; }
        return this.createRoomRequest(roomName, options, reuseRoomInstance, retryCount);
    };
    Client.prototype.rejoin = function (roomName, sessionId, reuseRoomInstance, retryCount) {
        return this.join(roomName, { sessionId: sessionId }, reuseRoomInstance, retryCount);
    };
    ...
    Client.prototype.connect = function (colyseusid, options, connectOptions) {
        var _this = this;
        if (options === void 0) { options = {}; }
        if (connectOptions === void 0) { connectOptions = {}; }
        this.id = colyseusid || '';
        this.connection = new Connection_1.Connection(this.buildEndpoint('', options), true, connectOptions);
